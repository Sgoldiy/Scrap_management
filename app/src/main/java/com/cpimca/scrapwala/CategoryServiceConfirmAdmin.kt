...
@Composable
fun CategoryServiceConfirmAdmin(navController: NavController) {
    val viewModel: ConfirmCategoryServiceViewModel = viewModel() // Initialize the view model

    // Fetch data when the composable is initially composed
    LaunchedEffect(Unit) {
        viewModel.fetchConfirmCetegoryPickupRequests()
    }


    // Observe the list of confirm requests from the view model
    val ConfirmCategoryPickupRequest by viewModel.ConfirmCetegoryPickupRequests.observeAsState(emptyList())
    ...
    if (isFetching) {
        ...
        ConfirmCategoryPickupRequestList(ConfirmCategoryPickupRequest = ConfirmCategoryPickupRequest)
    }
}

@Composable
fun ConfirmCategoryPickupRequestList(ConfirmCategoryPickupRequest: List<ConfirmCetegoryPickupRequest>) {
    LazyColumn(modifier = Modifier.fillMaxSize()) {
        items(ConfirmCategoryPickupRequest) { request ->
            ConfirmCategoryPickupRequestCard(request = request)
        }
    }
}

@Composable
fun ConfirmCategoryPickupRequestCard(request: ConfirmCetegoryPickupRequest) {
    Card(
        modifier = Modifier
            .padding(8.dp)
            .fillMaxWidth(),

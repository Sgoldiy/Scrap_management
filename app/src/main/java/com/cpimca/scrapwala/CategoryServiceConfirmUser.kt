...
@Composable
fun CategoryServiceConfirmUser(navController: NavController) {
    val viewModel: ConfirmCategoryServiceUserViewModel = viewModel() // Initialize the view model

    // Fetch data when the composable is initially composed
    LaunchedEffect(Unit) {
        viewModel.fetchConfirmCetegoryPickupRequestsForCurrentUser()
    }
    ...
    val ConfirmCetegoryCategoryRequests by viewModel.ConfirmCetegoryPickupRequests.observeAsState(emptyList())
    ...
    if (isFetching) {
        ...
        val userRequests = ConfirmCetegoryCategoryRequests.filter { it.email == loggedInUserEmail }
        ...
        if (userRequests.isEmpty()) {
            ...
            ConfirmCategoryRequestsListUser(ConfirmCategoryRequests = userRequests)
        }
    }
}
...
@Composable
fun ConfirmCategoryRequestsListUser(ConfirmCategoryRequests: List<ConfirmCetegoryPickupRequest>) {
    LazyColumn(
        modifier = Modifier
            .fillMaxSize()
            .padding(bottom = 60.dp , top = 80.dp)
    ) {
        items(ConfirmCategoryRequests) { request ->
            ConfirmCategoryRequestCardUser(request = request)
        }
    }
}

@Composable
fun ConfirmCategoryRequestCardUser(request: ConfirmCetegoryPickupRequest) {
    Card(
        modifier = Modifier
            .padding(8.dp)
            .fillMaxWidth(),
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            Text("Thank-you for your category service")
            // Name
            ...
            Row {
                ...
                Text(
                    "Category: ${request.selectedItem ?: "-"}", style = TextStyle(
                    )
                )
            }
            Row {
                ...
                Text(
                    "Time of category: ${request.selectedTime ?: "-"}", style = TextStyle(

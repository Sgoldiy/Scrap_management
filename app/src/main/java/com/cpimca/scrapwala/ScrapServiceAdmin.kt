...
@Composable
fun ScrapServiceAdmin(
    requests: List<ScrapServiceRequest>,
    isFetching: Boolean,
    onConfirm: (ScrapServiceRequest) -> Unit,
...
@Composable
fun ScrapServiceRequestCard(
    navController: NavController,
    request: ScrapServiceRequest,
    onConfirm: (ScrapServiceRequest) -> Unit,
) {
    Card(
        modifier = Modifier
        ...
        Column(
            ...
            Row {
                ...
                Text(
                    "Category: ${request.IteminKG ?: "-"}", style = TextStyle(
                    )
                )
            }
            Row {
...
@Composable
fun UserRequestScrap(
    viewModel: ScrapServiceViewModel = androidx.lifecycle.viewmodel.compose.viewModel(),
    navController: NavController,
) {

    // Define the onConfirm function
    val onConfirm: (ScrapServiceRequest) -> Unit = { request ->
        // Perform Firestore operations to transfer data from the original collection to the new collection
        // For example:
        val db = Firebase.firestore
        val confirmRequestsRef = db.collection("confirm_scrap_service_pickup")

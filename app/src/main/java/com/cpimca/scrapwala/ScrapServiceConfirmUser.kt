...
@Composable
fun ScrapServiceConfirmUser(navController: NavController) {
    val viewModel: ConfirmScrapServiceUserViewModel = viewModel() // Initialize the view model

    // Fetch data when the composable is initially composed
    LaunchedEffect(Unit) {
        viewModel.fetchConfirmScrapRequestsForCurrentUser()
    }
    ...
    val ConfirmCetegoryScrapRequests by viewModel.ConfirmScrapRequests.observeAsState(
        emptyList()
    )
    ...
    if (isFetching) {
        ...
        val userRequests = ConfirmCetegoryScrapRequests.filter { it.email == loggedInUserEmail }
        ...
        if (userRequests.isEmpty()) {
            ...
            ConfirmScrapRequestsListUser(ConfirmScrapRequests = userRequests)
        }
    }
}
...
@Composable
fun ConfirmScrapRequestsListUser(ConfirmScrapRequests: List<ConfirmScrapRequest>) {
    LazyColumn(
        modifier = Modifier
            .fillMaxSize()
            .padding(bottom = 60.dp, top = 80.dp)
    ) {
        items(ConfirmScrapRequests) { request ->
            ConfirmScrapRequestCardUser(request = request)
        }
    }
}

@Composable
fun ConfirmScrapRequestCardUser(request: ConfirmScrapRequest) {
    Card(
        modifier = Modifier
            .padding(8.dp)
            .fillMaxWidth(),
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            Row {
                ...
                Text(
                    "Scrap: ${request.iteminKG ?: "-"}", style = TextStyle(
                    )
                )
            }
            Row {
